...
reviews_df = pd.read_csv("C:/Users/Demir/Desktop/Bitirme_projesi/DataSets/Amazon_reviews.csv")   # read data   !!!!!
reviews_df_com = reviews_df[['Text','Score']]
reviews_df_com = reviews_df_com.dropna().copy() # boş yorum varsa kaldırır.
reviews_df_com = reviews_df_com.sample(frac = 0.005, replace = False, random_state=42) # Toplamda 15000 yarom var bunların %1 ini yani 1500 tanesini işleme koy.
...
if __name__ == "__main__":
    ...
    predict = []
    realDizi = reviews_df_com['Score'].values

    for t in range(len(reviews_df_com)):
        i = str(reviews_df_com['Text'].values[t])
        real = int(reviews_df_com['Score'].values[t])
        print("\n", i)
        sonuc = sentiment_scores(i) #sentiment_scores(clean_text(i))
        print("   User Rating :", real)
