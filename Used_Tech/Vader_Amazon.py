...
reviews_df = pd.read_csv("C:/Users/Demir/Desktop/Final_Project/DataSets/Amazon_reviews.csv")  # read data   !!!!!
reviews_df_com = reviews_df[['Text', 'Score']]
reviews_df_com = reviews_df_com.dropna().copy()  # boş yorum varsa kaldırır.
reviews_df_com = reviews_df_com.sample(frac = 0.005, replace = False, random_state=42) # Take %0.5 of 568.454 reviews
...
def clean_text(text):
    ...
    text = [WordNetLemmatizer().lemmatize(t[0], get_wordnet_pos(t[1])) for t in
            pos_tags]  # lemmatize text  -ing, -ed, s,ss,
